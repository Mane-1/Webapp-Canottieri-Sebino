...
async def crea_allenamento(
    ...
    selected_subgroups = db.query(models.SubGroup).filter(
        models.SubGroup.id.in_(subgroup_ids)).all() if subgroup_ids else db.query(models.SubGroup).filter(
        models.SubGroup.macro_group_id == macro_group_id).all()
